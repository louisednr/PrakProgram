...
//lcg class (made using struct not class)
struct LCG {
...
// Generates prime numbers p. 6 in PDF
...
int main() {
    LCG rng{166452}; // Initialise wit seed
    ...
    for (int N = 100; N <= 10000; N += 100) {
        ...
        // Reusing the exact same plainmc function for all three!
    ...
    // Define integrand incorporating the 1/pi^3 structural factor
    ...
    // Limits of integration: 0 to pi for all 3 dimensions
    std::vector<double> sing_min = {0.0, 0.0, 0.0};
    std::vector<double> sing_max = {M_PI, M_PI, M_PI};
    int N_sing = 500000; // Use a large N because it is a difficult singular integral
    ...
    // iii) Quasi-random sequence setup (Using 3D Halton Generator)
