...
double integrate(std::function<double(double)> f, double a, double b, 
    ...
    if (err < tol) {
        ...
        return integrate(f, a, (a + b) / 2.0, acc / std::sqrt(2.0), eps, f1, f2) +
               integrate(f, (a + b) / 2.0, b, acc / std::sqrt(2.0), eps, f3, f4);
    }
}
...
double erf(double z, double acc = 0.001, double eps = 0.001) {
    ...
    if (z >= 0 && z <= 1) {
        ...
        return (2.0 / std::sqrt(M_PI)) * integrate(f_inner, 0.0, z, acc, eps);
    } 
    // 1 < z
    ...
    return 1.0 - (2.0 / std::sqrt(M_PI)) * integrate(f_inner, 0.0, 1.0, acc, eps);
}
...
double clenshaw_curtis(std::function<double(double)> f, double a, double b, double acc = 0.001, double eps = 0.001) {
    ...
    return integrate(f_transformed, 0.0, M_PI, acc, eps);
}
...
int main() {
    ...
    ncalls = 0;
    // ---------- AI generated ----------
    double q1 = integrate([](double x){ return std::sqrt(x); }, 0.0, 1.0);
    ...
    double q2 = integrate([](double x){ return (x <= 0.0) ? 0.0 : 1.0 / std::sqrt(x); }, 0.0, 1.0);
    std::cout << "2) 1/sqrt(x)       | Calc: " << q2 << " | Exact: 2.000000000000 | Calls: " << ncalls << "\n";

    // Integral 3: sqrt(1 - x^2)
    ncalls = 0;
    double q3 = integrate([](double x){ return std::sqrt(1.0 - x * x); }, 0.0, 1.0);
    std::cout << "3) sqrt(1-x^2)     | Calc: " << q3 << " | Exact: " << M_PI / 2.0 << " | Calls: " << ncalls << "\n";

    // Integral 4: ln(x)/sqrt(x)
    ncalls = 0;
    double q4 = integrate([](double x){ return (x <= 0.0) ? 0.0 : std::log(x) / std::sqrt(x); }, 0.0, 1.0);
    ...
    ncalls = 0;
    double res1_ord = integrate(f1, 0.0, 1.0, 1e-6, 1e-6);
    ...
    ncalls = 0;
    double res2_ord = integrate(f2, 0.0, 1.0, 1e-6, 1e-6);
