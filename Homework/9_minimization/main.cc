...
int main() {
    ...
    pp::vector<double> ros_start = {0.5, 0.5};
    ...
    // Read the  data table from the standard input
    std::vector<double> energy;
    std::vector<double> signal;
    std::vector<double> error;

    std::string header;
    std::getline(std::cin, header); // read and discard the header line
    ...
    std::cout << "Successfully loaded " << energy.size() << " data points from stdin.\n";
    ...
    std::cout << "\n=== Fit Results ===\n";
    std::cout << "Calculated Higgs mass (m):     " << fitted_mass << " GeV/c^2\n";
    std::cout << "Experimental width (Gamma):   " << fitted_width << " GeV\n";
    std::cout << "Fitted scale factor (A):       " << fitted_scale << "\n";
    std::cout << "Final residual variance value: " << deviation_D(p_fit) << "\n";
    ...
    std::cout << "--- Minimizing Rosenbrock's valley function (Central) ---\n";
    std::cout << "Starting guess: " << ros_start << "\n";
    
    // Run the new central difference minimizer
    pp::vector<double> ros_min_central = pp::minimize_newton_central(rosenbrock, ros_start);
    std::cout << "Calculated minimum location: " << ros_min_central << "\n";
    std::cout << "Function value at minimum:   " << rosenbrock(ros_min_central) << "\n\n";

    // AI generated table format below
    // --- Performance Comparison Table ---
    std::cout << "====================================================================\n";
    std::cout << "              PERFORMANCE COMPARISON: ROSENBROCK                    \n";
    std::cout << "====================================================================\n";
    std::cout << "Metric              | Forward (Part A)      | Central (Part C)      \n";
    std::cout << "--------------------|-----------------------|-----------------------\n";
    std::cout << "Minimum X           | " << ros_min[0] << "          | " << ros_min_central[0] << "\n";
    std::cout << "Minimum Y           | " << ros_min[1] << "          | " << ros_min_central[1] << "\n";
    std::cout << "Final f(x,y) Value  | " << rosenbrock(ros_min) << "           | " << rosenbrock(ros_min_central) << "\n";
    std::cout << "====================================================================\n\n";

    std::cout << "So each method has the same number of iterations, but central difference get a lower minimum value";
